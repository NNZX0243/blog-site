...
;(() => {
	 ...
	 */
	const readVersion = () =>
		caches.match(CTRL_PATH).then((response) => response?.json())
	/**
	 * 写入版本号
	...
	const writeVersion = (version) =>
		caches
			.open(CACHE_NAME)
			.then((cache) =>
				cache.put(CTRL_PATH, new Response(JSON.stringify(version)))
			)
				...
				return event.respondWith(
					new Promise((resolve, reject) => {
						cacheMap
							.get(cacheKey)
							.push((arg) =>
								arg.body ? resolve(arg) : reject(arg)
							)
					})
				)
			}
		...
		if (cacheRule) {
			...
			handleFetch(
				caches.match(key).then(
					(cache) =>
						cache ??
						fetchFile(request, true).then((response) => {
							if (checkResponse(response)) {
								const clone = response.clone()
								caches
									.open(CACHE_NAME)
									.then((it) => it.put(key, clone))
								// [debug put]
							}
							return response
						})
			...
			// [modifyRequest else-if]
				...
				handleFetch(
					fetchWithCache(request).catch(
						(err) => new Response(err, { status: 499 })
					)
				)
		}
	})
	...
	const updateJson = async () => {
		...
		const parseJson = (json) =>
			readVersion().then((oldVersion) => {
				...
				const newVersion = {
					global,
					local: info[0].version,
					escape: oldVersion?.escape ?? 0,
				}
	...
	function CacheChangeExpression(json) {
		...
		const getMatch = () => {
			switch (json['flag']) {
				...
				case 'end':
					return (url) =>
						forEachValues((value) => url.href.endsWith(value))
				case 'begin':
					return (url) =>
						forEachValues((value) => url.pathname.startsWith(value))
				case 'str':
					return (url) =>
						forEachValues((value) => url.href.includes(value))
				case 'reg':
					// noinspection JSCheckFunctionSignatures
					return (url) =>
						forEachValues((value) =>
							url.href.match(new RegExp(value, 'i'))
						)
				default:
